...
/// Floor so a lucky ping does not underrun the pose buffer (~3 ticks @ 128 Hz).
pub const REMOTE_INTERP_DELAY_MIN_SECS: f32 = 0.024;
...
mod tests {
    ...
    fn ack_samples_rtt_and_clamps_delay() {
        ...
        // 20 ms RTT → 0.5 * 0.02 = 0.01 → clamp to MIN 24 ms
        lag.on_ack(1, 1.020);
        let rtt = lag.rtt_ema().unwrap();
        assert!((rtt - 0.020).abs() < 1e-5, "rtt={rtt}");
        assert!(
