...
fn table_covers_a_through_r() {
    ...
    assert_eq!(weapon_def(b'a').unwrap().mag_capacity(), 0);
    assert_eq!(weapon_def(b'e').unwrap().mag_capacity(), 6);
    assert_eq!(weapon_def(b'i').unwrap().mag_capacity(), 0);
    assert_eq!(weapon_def(b'j').unwrap().mag_capacity(), 0);
