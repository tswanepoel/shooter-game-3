...
Depends on **038** (fire gates), **044** / **045** (hit marker, hit impulse), **058** (magazine / reserve / reload), **060** (remote display names), **063** (product surfaces), **074** (pump voice / reload slap), **075** (hit marker SFX).
...
An asked-for [reload](../concepts.md#reload) now takes handling time before it fills the [magazine](../concepts.md#magazine). No [fire](../concepts.md#fire) is accepted while rounds are being loaded — by an asked-for reload or by a **pump**. Swapping the [active slot](../concepts.md#active-slot), spawning, or dying cancels a reload in flight.

| Mode | Handling time |
| --- | --- |
| Pump letters | **074** pump time (base + per-round) |
...
**Chamber** = seated store (noun). **Pump** = timed seat that blocks fire and plays the pump voice (verb). Never call the verb “chamber.”

### Per blaster

|   | class    | mode  | muz | chamber | pump | mag | load | spare |
| - | -------- | ----- | --: | ------: | ---: | --: | ---: | ----: |
| a | launcher | semi  |   1 |       1 |    1 |   — |    — |     2 |
| b | pistol   | semi  |   1 |       1 |    — |  12 |    8 |    30 |
| c | smg      | auto  |   1 |       1 |    — |  24 |   20 |    60 |
| d | ar       | burst |   1 |       1 |    — |  24 |   18 |    48 |
...
- Letter table owns `chamber`, `pump` (count), `mag`, `load`, `spare`.
...
- `pump` — / `0`: seat from mag (or reserve when no mag) with no timed pump / no pump voice.
- `pump` N: N rounds go into the chamber, one timed seat at a time; pump voice once per seat.
- No-mag (`a` `i` `j` `o`): `mag` —; chamber / pump 1 / 2 / 2 / 4; refill from reserve starts only when empty, so `i` fires both seats before a pump.
- SFX / APIs: verb is **pump** (`pump-a.wav`…); not “chamber.”
...
Pump / breech envelope (sim emits start → seat×N → end; present picks the style):
...
- Pump letters still pump (**074**) and block fire until the seat lands.
